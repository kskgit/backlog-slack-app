  ...
  override def registrationIssueToBacklog: ViewSubmissionHandler = (req, ctx) =>
    {
      ...
      val projectId =
        getViewValues.get("pjId").get("acId").getSelectedOption.getValue
    ...
    projects.forEach(p =>
      options.add(
        OptionObject
          .builder()
          .value(
            "projectId:" + p.getProjectId.toString + "," + "issueTypeId:" + p.getId.toString
          )
          .text(PlainTextObject.builder().text(p.getName).build())
          .build()
      )
